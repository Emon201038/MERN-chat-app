...
const handleCreateComment = async (req, res, next) => {
  try {
    ...
    if (text && !file) {
      update.$push.comments.comment.text = text;
    }
    if (text && file) {
      update.$push.comments.comment.text = text;
      update.$push.comments.comment.image = file;
    }
    if (!text && file) {
      update.$push.comments.comment.image = file;
    }

    const post = await Post.findByIdAndUpdate(postId, update, updateOptions);

    if (!post) {
      throw createError(500, "Comment cannot be added");
    }
...
const handleDeleteComment = async (req, res, next) => {
  try {
    ...
    // Check if the user is the post owner or the commenter
    if (
